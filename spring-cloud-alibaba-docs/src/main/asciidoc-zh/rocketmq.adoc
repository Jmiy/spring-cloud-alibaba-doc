...
下载 https://mirror-hk.koddos.net/apache/rocketmq/4.9.0/rocketmq-all-4.9.0-bin-release.zip[RocketMQ最新的二进制文件]，并解压
...
└── lib
```
* 创建提交日志目录，用于存储 提交日志文件
```bash
mkdir ${user.home}/store/commitlog
chmod -R 777 commitlog
```

* 创建消费队列目录，用于存储 客户端消费队列的相对位置
```bash
mkdir ${user.home}/store/consumequeue
chmod -R 777 consumequeue
```
